$(document).ready(function(){
  ...
  // Event Two
  var eventNameTwo = "Fantasy of Lights Walk Through - Stroller Elves Shift";
  firebase.database().ref("events/" + eventNameTwo + "/date").once("value").then(function(snapshot) {
    var date = snapshot.val();
    firebase.database().ref("events/" + eventNameTwo + "/description").once("value").then(function(snapshot) {
      var description = snapshot.val();
      firebase.database().ref("events/" + eventNameTwo + "/hours").once("value").then(function(snapshot) {
        var hours = snapshot.val();
        firebase.database().ref("events/" + eventNameTwo + "/location").once("value").then(function(snapshot) {
          var location = snapshot.val();
          firebase.database().ref("events/" + eventNameTwo + "/picture").once("value").then(function(snapshot) {
            var picture = snapshot.val();
            firebase.database().ref("events/" + eventNameTwo + "/time").once("value").then(function(snapshot) {
              var time = snapshot.val();
              //console.log(date+" "+description+" "+hours+" "+location+" "+picture+" "+time);
              // Show which members are going
              var membersRef = firebase.database().ref().child("events/"+eventNameTwo+"/members");
              var memberList = "";
              membersRef.once("value", function(snapshot) {
                ...
                $("#appendEvents").append("<div class=\"row\"><div class=\"col-md-7\"><a href=\"\"><img id=\"imageTwo\"class=\"img-fluid rounded mb-3 mb-md-0\" src=\"" + picture + "\" alt=\"Loading image...\"> </a></div><div class=\"col-md-5\"><h3 id=\"titleTwo\">" + eventNameTwo + "</h3> <p id=\"descriptionTwo\">" + eventInfo + "</p> <a class=\"btn btn-primary\" style=\"color: white\" onclick=\"signup('Fantasy of Lights Walk Through - Stroller Elves Shift')\">Sign Up for Event <span class=\"glyphicon glyphicon-chevron-right\"></span></a></div></div>");
